...
#+BEGIN_SRC emacs-lisp
  ...
  (defun dgit ()
    "Run magit in dotfiles repo"
    (interactive)
    (with-temp-buffer
      (let ((magit-git-environment (copy-sequence magit-git-environment))
            (home (getenv "$HOME")))
        (setenv-internal magit-git-environment "GIT_DIR" (concat home "/.dotcfg") t)
        (setenv-internal magit-git-environment "GIT_WORK_TREE" home t)
        (magit))))
#+END_SRC
